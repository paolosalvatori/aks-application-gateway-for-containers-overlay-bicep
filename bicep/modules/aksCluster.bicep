...
resource systemAgentPoolSubnet 'Microsoft.Network/virtualNetworks/subnets@2024-03-01' existing = {
  parent: virtualNetwork
  name: systemAgentPoolSubnetName
}

resource userAgentPoolSubnet 'Microsoft.Network/virtualNetworks/subnets@2024-03-01' existing = {
  parent: virtualNetwork
  name: userAgentPoolSubnetName
}

resource podSubnet 'Microsoft.Network/virtualNetworks/subnets@2024-03-01' existing = if (networkPlugin != 'overlay') {
  parent: virtualNetwork
  name: podSubnetName
}

resource apiServerSubnet 'Microsoft.Network/virtualNetworks/subnets@2024-03-01' existing = {
  parent: virtualNetwork
  name: apiServerSubnetName
}

resource dnsZone 'Microsoft.Network/dnsZones@2023-07-01-preview' existing = if (webAppRoutingEnabled && !empty(dnsZoneName) && !empty(dnsZoneResourceGroupName)) {
  name: dnsZoneName
